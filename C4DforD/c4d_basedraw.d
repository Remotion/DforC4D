...
{
private:
	@disable this();  //static BaseDrawHelp opCall();
	//~this();

public:
	BaseDocument* GetDocument()        { return C4DOS.Br.BbGetDocument(&this); }
	BaseTag* GetActiveTag()            { return C4DOS.Br.BbGetActiveTag(&this); }
	ref const Matrix 	GetMg()        { return C4DOS.Br.BbGetMg(&this); }
	BaseContainer	GetDisplay()       { BaseContainer bc; C4DOS.Br.BbGetDisplay(&this, &bc); return bc; }
	void SetDisplay(BaseContainer* bc)  { C4DOS.Br.BbSetDisplay(&this, bc); }
	void SetMg(ref const Matrix  mg)        { C4DOS.Br.BbSetMg(&this, mg); }
	...
	static void Free(ref BaseDrawHelp*  p);
};


//=============================================================================
struct BaseView //: public BaseList2D
{
	BaseList2D parent; //?
	alias parent this; //? 
private:
	@disable this(); //BaseView();
	//~BaseView();
...
public:
	...
	Matrix GetMg() { return C4DOS.Bv.GetMg(&this); }
	Matrix GetMi() { return C4DOS.Bv.GetMi(&this); }
	ref Matrix  GetBaseMatrix() const { return C4DOS.Bv.GetBaseMatrix(&this); }
	void SetBaseMatrix(ref const Matrix  m) { C4DOS.Bv.SetBaseMatrix(&this, m); }
	...
	Int32 GetProjection() { return C4DOS.Bv.GetProjection(&this); }
	Bool TestPoint(Float x, Float y) { return C4DOS.Bv.TestPoint(&this, x, y); }
	Bool TestPointZ(ref const Vector  p) { return C4DOS.Bv.TestPointZ(&this, p); }
	Bool TestClipping3D(ref const Vector  mp, ref const Vector  rad, ref const Matrix  mg, Bool* clip2d, Bool* clipz);
	...
	Vector WS(ref const Vector  p) { return C4DOS.Bv.WS(&this, p); }
	Vector SW(ref const Vector  p) { return C4DOS.Bv.SW(&this, p); }
	Vector SW_Reference(Float x, Float y, ref const Vector  wp) { return C4DOS.Bv.SW_R(&this, x, y, wp); }
	Vector WC(ref const Vector  p) { return C4DOS.Bv.WC(&this, p); }
	Vector CW(ref const Vector  p) { return C4DOS.Bv.CW(&this, p); }
	Vector SC(ref const Vector  p) { return C4DOS.Bv.SC(&this, p); }
	Vector CS(ref const Vector  p, Bool z_inverse) { return C4DOS.Bv.CS(&this, p, z_inverse); }
	Vector WC_V(ref const Vector  v) { return C4DOS.Bv.WC_V(&this, v); }
	Vector CW_V(ref const Vector  v) { return C4DOS.Bv.CW_V(&this, v); }
	Bool BackfaceCulling(ref const Vector  n, ref const Vector  p) { return C4DOS.Bv.BackfaceCulling(&this, n, p); }
	Bool ZSensitiveNear() { return C4DOS.Bv.ZSensitiveNear(&this); }
	Float ZSensitiveNearClipping() { return C4DOS.Bv.ZSensitiveNearClipping(&this); }
	StereoCameraInfo* GetStereoInfo() const { return C4DOS.Bv.GetStereoInfo(&this); }
	Bool ZSensitiveFar() { return C4DOS.Bv.ZSensitiveFar(&this); }
	Float ZSensitiveFarClipping() { return C4DOS.Bv.ZSensitiveFarClipping(&this); }
	...
	Float PW_W(ref const Vector  p, Bool horizontal);	//Pixel Unit at world position to World Unit
	Float WP_W(ref const Vector  p, Bool horizontal);	//World Unit at world position to Pixel Unit

	Vector ProjectPointOnLine(ref const Vector  p, ref const Vector  v, Float mouse_x, Float mouse_y, Float* offset = nullptr, Int32* err = nullptr);
	Vector ProjectPointOnPlane(ref const Vector  p, ref const Vector  v, Float mouse_x, Float mouse_y, Int32* err = nullptr);
};

//=============================================================================
struct BaseDraw //: public BaseView
{
	BaseView parent; //?
	alias parent this; //? 
private:
	@disable this(); //BaseDraw();
	//~BaseDraw();
/+
...
public:
	...
	static void Free(ref BaseDraw*  bd) { C4DOS.Br.FreeBaseDraw(bd); }
	...
	Bool HasCameraLink() { return C4DOS.Br.HasCameraLink(&this); }
	...
	BaseObject* GetEditorCamera()  { return C4DOS.Br.GetEditorCamera(&this); }
	...
	enum INIT_CLIPBOX_ADJUSTGLSIZE =	1;
	...
	void InitView(BaseContainer* camera, ref const Matrix  op_m, Float sv, Float pix_x, Float pix_y, Bool fitview) { C4DOS.Br.InitView(&this, camera, op_m, sv, pix_x, pix_y, fitview); }
	...
	Vector CheckColor(ref const Vector  col)  { return C4DOS.Br.CheckColor(&this, col); }

	void SetTransparency(Int32 trans)  { C4DOS.Br.SetTransparency(&this, trans); }
	Int32 GetTransparency()  { return C4DOS.Br.GetTransparency(&this); }

	Bool PointInRange(ref const Vector  p, Int32 x, Int32 y)  { return C4DOS.Br.PointInRange(&this, p, x, y); }
	void SetPen(ref const Vector  col, Int32 flags = 0)  { C4DOS.Br.SetPen(&this, col, flags); }
	void SetPointSize(Float pointsize) { C4DOS.Br.SetPointSize(&this, pointsize); }
	Vector ConvertColor(ref const Vector  c) { return C4DOS.Br.ConvertColor(&this, c); }
	Vector ConvertColorReverse(ref const Vector  c) { return C4DOS.Br.ConvertColorReverse(&this, c); }
	Float SimpleShade(ref const Vector  p, ref const Vector  n)  { return C4DOS.Br.SimpleShade(&this, p, n); }
	...
	void SetMatrix_Matrix(BaseObject* op, ref const Matrix  mg) { C4DOS.Br.SetMatrix_Matrix(&this, op, mg); }														// sets the transformation matrix to the given matrix
	void SetMatrix_Matrix(BaseObject* op, ref const Matrix  mg, Int32 zoffset) { C4DOS.Br.SetMatrix_MatrixO(&this, op, mg, zoffset); }	// sets the transformation matrix to the given matrix
	void SetClipPlaneOffset(Float o) { C4DOS.Br.SetClipPlaneOffset(&this, o); }
	
	// flags for DrawLine,LineStrip,DrawHandle,DrawPoly,...
	enum NOCLIP_D = 1;	// clip against the view border
	enum NOCLIP_Z = 2;	// z clipping

	void LineStripBegin() { C4DOS.Br.LineStripBegin(&this); }
	void LineStrip(ref const Vector  vp, ref const Vector  vc, Int32 flags) { C4DOS.Br.LineStrip(&this, vp, vc, flags); }
	void LineStripEnd() { C4DOS.Br.LineStripEnd(&this); }

	void DrawPoint2D(ref const Vector  p)  { C4DOS.Br.DrawPoint2D(&this, p); }
	void DrawLine2D(ref const Vector  p1, ref const Vector  p2)  { C4DOS.Br.DrawLine2D(&this, p1, p2); }
	void DrawHandle2D(ref const Vector  p, DRAWHANDLE type = DRAWHANDLE_SMALL) { C4DOS.Br.DrawHandle2D(&this, p, type); }
	void DrawCircle2D(Int32 mx, Int32 my, Float rad) { C4DOS.Br.DrawCircle2D(&this, mx, my, rad); }
	void DrawHandle(ref const Vector  vp, DRAWHANDLE type, Int32 flags) { C4DOS.Br.DrawHandle(&this, vp, type, flags); }
	...
	void DrawLine(ref const Vector  p1, ref const Vector  p2, Int32 flags) { C4DOS.Br.DrawLine(&this, p1, p2, flags); }
	void DrawArc(ref const Vector  pos, Float radius, Float angle_start, Float angle_end, Int32 subdiv = 32, Int32 flags = 0) { C4DOS.Br.DrawArc(&this, pos, radius, angle_start, angle_end, subdiv, flags); }
	...
	void DrawCircle(ref const Matrix  m) { C4DOS.Br.DrawCircle(&this, m); }
	void DrawBox(ref const Matrix  m, Float size, ref const Vector  col, Bool wire) { C4DOS.Br.DrawBox(&this, m, size, col, wire); }
	void DrawPolygon(Vector* p, Vector* f, Bool quad) { C4DOS.Br.DrawPolygon(&this, p, f, quad); }
	void DrawSphere(ref const Vector  off, ref const Vector  size, ref const Vector  col, Int32 flags) { C4DOS.Br.DrawSphere(&this, off, size, col, flags); }
	
	enum BDRAW_DRAW_SPHERE_FLAGS_NO_SHADING = 1;

	void DrawArrayEnd() { C4DOS.Br.DrawArrayEnd(&this); }
	DRAWRESULT DrawPolygonObject(BaseDrawHelp* bh, BaseObject* op, DRAWOBJECT flags, BaseObject* parent = nullptr,in Vector col = Vector(0.5)) { return C4DOS.Br.DrawPObject(&this, bh, op, flags, DRAWPASS_OBJECT, parent, col); }
	DRAWRESULT DrawObject(BaseDrawHelp* bh, BaseObject* op, DRAWOBJECT flags, DRAWPASS drawpass, BaseObject* parent = nullptr, in Vector col = Vector(0.5)) { return C4DOS.Br.DrawPObject(&this, bh, op, flags | DRAWOBJECT_PRIVATE_ANY, drawpass, parent, col); }
	...
	Bool GetHighlightPassColor(ref const BaseDrawHelp  bh, Bool lineObject, ref Vector  col) const { return C4DOS.Br.GetHighlightPassColor(&this, bh, lineObject, col); }
	GlFrameBuffer* GetHighlightFramebuffer(/*ref const*/in Vector32  vMin = Vector32(-1.0f), /*ref const*/in Vector32  vMax = Vector32(1.0f)) { return C4DOS.Br.GetHighlightFramebuffer(&this, vMin, vMax); }
	...
	enum BDRAW_SETLIGHTLIST_NOLIGHTS =			-1;
	enum BDRAW_SETLIGHTLIST_SCENELIGHTS =	0;
	enum BDRAW_SETLIGHTLIST_QUICKSHADING =	1;

	void InitUndo(BaseDocument* doc) { C4DOS.Br.InitUndo(&this, doc); }
	void DoUndo(BaseDocument* doc) { C4DOS.Br.DoUndo(&this, doc); }

	void SetDrawParam(Int32 id, ref const GeData data) { C4DOS.Br.SetDrawParam(&this, id, data); }
	GeData GetDrawParam(Int32 id) { return C4DOS.Br.GetDrawParam(&this, id); }

	enum DRAW_PARAMETER_LINEWIDTH =				1000;	// [SET/GET] Float LineWidth
	enum DRAW_PARAMETER_ALPHA_THRESHOLD =	1002;	// [SET/GET] Float 0 ... 1.0 (make sure to set old value after change!
	enum DRAW_PARAMETER_SETZ =							2;
	enum DRAW_Z_LOWEREQUAL =	0;
	enum DRAW_Z_GREATER =		1;
	enum DRAW_Z_ALWAYS =			2;
	enum DRAW_Z_EQUAL =			3;
	enum DRAW_Z_LOWER =			4;
	enum DRAW_PARAMETER_OGL_CULLING =			10;
	enum DRAW_PARAMETER_OGL_CULLING_OFF =				0;
	enum DRAW_PARAMETER_OGL_CULLING_FRONT =			1;
	enum DRAW_PARAMETER_OGL_CULLING_BACK =				2;
	enum DRAW_PARAMETER_OGL_CULLING_FRONT_AND_BACK = 3;
	enum DRAW_PARAMETER_USE_Z =						11;
	enum DRAW_PARAMETER_OGL_PRIMITIVERESTARTINDEX =	14; //UInt16

	// use with BASEDRAW_DRAWPORTTYPE
	enum DRAWPORT_TYPE_SOFTWARE = 0;
	//#define DRAWPORT_TYPE_OGL				1
	enum DRAWPORT_TYPE_OGL_HQ = 2;

	Bool TestBreak() { return C4DOS.Br.TestBreak(&this); }
	ref OITInfo  GetOITInfo() { return C4DOS.Br.GetOITInfo(&this); }
	Bool GetFullscreenPolygonVectors(ref Int32  lAttributeCount, ref const GlVertexBufferAttributeInfo**  ppAttibuteInfo, ref Int32  lVectorInfoCount, ref const GlVertexBufferVectorInfo**  ppVectorInfo)
	{ return C4DOS.Br.GetFullscreenPolygonVectors(&this, lAttributeCount, ppAttibuteInfo, lVectorInfoCount, ppVectorInfo); }
	
	Bool DrawFullscreenPolygon(Int32 lVectorInfoCount, const GlVertexBufferVectorInfo** ppVectorInfo)
	...
	GlLight* GetGlLight(Int32 lIndex) const { return C4DOS.Br.GetGlLight(&this, lIndex); }
	Bool GetDrawStatistics(ref BaseContainer  bc) const { return C4DOS.Br.GetDrawStatistics(&this, bc); }
	DRAWPASS GetDrawPass() const { return C4DOS.Br.GetDrawPass(&this); }

	ref Matrix4d  GetViewMatrix(Int32 n);
	...
	void GetGridStep(ref Float  step, ref Float  fade) { C4DOS.Br.GetGridStep(&this, step, fade); }
...
public:
	BaseDocument* GetDocument()             { return C4DOS.Br.BbGetDocument(this); }
	BaseTag* GetActiveTag()            { return C4DOS.Br.BbGetActiveTag(this); }
	const Matrix&	GetMg()                    { return C4DOS.Br.BbGetMg(this); }
	BaseContainer	GetDisplay()               { BaseContainer bc; C4DOS.Br.BbGetDisplay(this, &bc); return bc; }
	void SetDisplay(BaseContainer* bc)  { C4DOS.Br.BbSetDisplay(this, bc); }
