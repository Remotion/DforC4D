...
import c4d;
import c4d_file;
import c4d_commanddata;
import c4d_general;

import c4d_gui;
import c4d_resource;

import gradientuserarea;
import c4d_symbols;
...
//=========================================================
class SDKGradientArea : GeUserArea
{
public:
	SDKGradientGadget ggtmp; // = null;
	SDKGradient		  grad[MAXGRADIENT];
	Int32			  count = 2, interpolation = 4, type = 0;

public:

	this() {
		//printf(" >>> SDKGradientArea.this %p \n",this);

		ggtmp = SDKGradientGadget(IC);
	...
	override Bool Init()
	{
		//printf(" >>> SDKGradientArea.Init %p \n",this);
		ggtmp.Init(this, grad, &count, &interpolation, MAXGRADIENT);
		return true;
	}

	override Bool GetMinSize(ref Int32  w, ref Int32  h)
	{
		w = 100;
		h = 200;
		return true;
	...
	override void Sized(Int32 w, Int32 h) {
		ggtmp.InitDim(w, h);
	}

	override void DrawMsg(Int32 x1, Int32 y1, Int32 x2, Int32 y2, ref const BaseContainer  msg){
...
private:
	...
	SDKGradientArea sg = null;
	C4DGadget*		gradientarea = null;
...
public:
	this(){	
		sg = new SDKGradientArea; //! new !!!
		//printf(" >>> AsyncDialog.this %p %p \n",this,sg);
		gradientarea = null;
		rows = 1;
	}
	...
	override Bool CreateLayout()
	{
		...
		SetTitle(String("GuiDemo C++"));


		GroupBegin(0, BFH_SCALEFIT, 5, 0, String(""), 0);
		{
			GroupBorderSpace(4, 4, 4, 4);
			AddButton(GADGET_ADDROW, BFH_FIT, 0, 0, String("add row"));
			AddButton(GADGET_SUBROW, BFH_FIT, 0, 0, String("sub row"));
		}
		GroupEnd();

		GroupBegin(0, BFH_SCALEFIT, 2, 0, String(""), 0);
		{
			GroupBegin(0, BFV_SCALEFIT | BFH_SCALEFIT, 0, 1, String("Drop objects, tags, materials here"), 0);
			{
			...
			GroupBegin(0, BFV_SCALEFIT | BFH_SCALEFIT, 0, 2, String(""), 0);
			{
				...
				if (gradientarea != null) {
					Bool ar = AttachUserArea(sg, GadgetPtr(gradientarea));
				} //else GePrint(" ! AttachUserArea ");
				GroupBegin(0, BFH_LEFT, 2, 0, String(""), 0);
				{
					...
					import c4d_os : IconData;
					import lib_iconcollection;
					...
					AddChild(IDC_XINTERPOL, 0, GeLoadString(IDS_NONE) + String("&") + String.HexToString(cast(UInt) &dat1) + String("&"));
					AddChild(IDC_XINTERPOL, 1, GeLoadString(IDS_LINEAR) + String("&") + String.HexToString(cast(UInt)  &dat2) + String("&"));
					AddChild(IDC_XINTERPOL, 2, GeLoadString(IDS_EXPUP) + String("&") + String.HexToString(cast(UInt)  &dat3) + String("&"));
					AddChild(IDC_XINTERPOL, 3, GeLoadString(IDS_EXPDOWN) + String("&") + String.HexToString(cast(UInt)  &dat4) + String("&"));
					AddChild(IDC_XINTERPOL, 4, GeLoadString(IDS_SMOOTH) + String("&i") + String.IntToString(Tphong) + String("&"));	// use Icon ID
		...
		MenuFlushAll();
		MenuSubBegin(String("Menu1"));
		MenuAddString(GADGET_ADDROW, String("add row"));
		MenuAddString(GADGET_SUBROW, String("sub row"));
		MenuAddString(GADGET_R1, String("test1&c&"));
		MenuAddString(GADGET_R2, String("test2&d&"));
		MenuAddSeparator();
		MenuSubBegin(String("SubMenu1"));
		MenuAddCommand(1001153);	// atom object
		MenuAddCommand(1001157);	// rounded tube object
		MenuAddCommand(1001158);	// spherify object
		MenuSubBegin(String("SubMenu2"));
	...
	void ReLayout()
	{
		LayoutFlushGroup(GadgetPtr(GROUP_DYNAMIC));
		CreateDynamicGroup();
		LayoutChanged(GadgetPtr(GROUP_DYNAMIC));
	}
	...
	override Bool InitValues()
	{
		...
		SetInt32(GadgetPtr(IDC_OFFSET), 100, 0, 100, 1);
		SetBool(GadgetPtr(IDC_ACCESS), true);

		Float pos = 0.0;
		sg.ggtmp.GetPosition(&pos);

		SetPercent(GadgetPtr(IDC_XPOSITION), pos);
		SetInt32(GadgetPtr(IDC_XINTERPOL), sg.interpolation);
	...
	Bool CheckDropArea(Int32 id, ref const BaseContainer  msg)
	{
		Int32 x, y, w, h, dx, dy;
	...
	void CreateDynamicGroup()
	{
		...
		for (i = 0; i < rows; i++)
		{
			AddCheckbox(0, BFH_LEFT, 0, 0, String("Rows ") + String.IntToString(i + 1));
			AddStaticText(GadgetPtr(GADGET_DRAG + i), BFH_SCALEFIT, 260, 0, GetStaticText(i), BORDER_THIN_IN);
	...
	override Bool Command(Int32 id, ref const BaseContainer  msg)
	{
		switch (id)
		{
			default:   //! CRASH if we have no DEFAULT  !
				break;
			case GADGET_ADDROW:
				if (rows < 100)
					rows++;
	...
	static String GenText(C4DAtomGoal* bl)
	{
		...
		if (bl.IsInstanceOf(Tbaselist2d))
			return str + " " + (cast(BaseList2D*)bl).GetName() + " (" + String.IntToString(bl.GetType()) + ")";
		...
		switch (id)
		{
			...
			case EVMSG_CHANGE:
				if (CheckCoreMessage(msg))
				{
					Int32 i;
					for (i = 0; i < rows; i++)	{
						SetString(GadgetPtr(GADGET_DRAG + i), GetStaticText(i));
					}
				}
				break;
		}
	...
	override Int32 Message(ref const BaseContainer  msg, ref BaseContainer  result)
	{
		switch (msg.GetId())
		{
			default:   //! CRASH if we have no DEFAULT  !
			...
			case BFM_GETCURSORINFO:
				{
					...
					HandleHelpString(msg, result, String("PLUGIN_CMD_???"));
					break;
				}
			...
			case BFM_DRAGRECEIVE:
				{
					...
					if (id != -1)
					{
						...
						else
						{
							...
							if (type == DRAGTYPE_ATOMARRAY && (cast(AtomArray*)object).GetCount() == 1 && (cast(AtomArray*)object).GetIndex(0))
							{
								bl = cast(C4DAtomGoal*)(cast(AtomArray*)object).GetIndex(0);
								if (bl)
								{
									if (bl.IsInstanceOf(Obase))
									{
									...
									if (bl.IsInstanceOf(Tbaselist2d))
										string = prefix + str + " " + (cast(BaseList2D*)bl).GetName() + " (" + String.IntToString(bl.GetType()) + ")";
									else
							...
							else if (type == DRAGTYPE_BROWSER)
							{
								...
								bdi = cast(SDKBrowserDragInfo*) object;
								if (bdi)
								{
									Int32	cnt;
...
enum ID_ASYNCTEST = 1010955;

//==============================================
class AsyncTest : CommandData
{
private:
	AsyncDialog dlg;
...
public:
	...
	override Int32 GetState(BaseDocument* doc)
	{
		return CMD_ENABLED;
	}
...
{
	AsyncTest tmp = new AsyncTest();  //this looks BAD !!!
	return RegisterCommandPlugin(ID_ASYNCTEST, GeLoadString(IDS_ASYNCTEST), 0, nullptr, String("C++ SDK Menu Test Plugin"), /*cast(CommandData*)*/tmp);
